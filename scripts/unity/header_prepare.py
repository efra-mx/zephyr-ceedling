...
def header_prepare(in_file, out_file, out_wrap_file, prefix):
    ...
    # Prepare file with functions prefixed with 'prefix' that will be used for
    # mock generation.
    func_pattern = re.compile(
        r"^\s*((?:\w+[*\s]+)+)(\w+?\s*\([\w\s,*\.\[\]]*?\)\s*;)", re.M)
    content2 = func_pattern.sub(r"\n\1" + re.escape(prefix) + r"\2", content)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-wp", "--prefix", type=str, default='__mock_',
                        help='wrap prefix for mocked functions')
    args = parser.parse_args()

    header_prepare(args.input, args.output, args.wrap, args.prefix)